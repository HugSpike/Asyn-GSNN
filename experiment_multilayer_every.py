...
class Graph_Net(nn.Module):
    ...
    def forward(self, data):
        ...
        gcn_matrices.append(gcn_matrix1)
        fused_features.append(fused_features1)
        mu0.append(mu_0)
        mu1.append(mu_1)
        logvar0.append(logvar_0)
        logvar1.append(logvar_1)

        if data.spike_node is None:
            out = self.classifier1(fused_transfered_features1)
            return gcn_matrices, out, fused_features, mu0, logvar0, mu1, logvar1
        else:
            ...
            mapped_snn_matrix2, mu_01, logvar_01, mu_11, logvar_11 = self.spi_vae2(spike_matrix2)
            mapped_snn_matrix2 = mapped_snn_matrix2.view(fusion_matrix2.shape[0], fusion_matrix2.shape[1])

            fused_features2 = mapped_snn_matrix2 + fusion_matrix2

            fused_transfered_features2 = self.fusion_transfer2(fused_features2)

            gcn_matrices.append(gcn_matrix2)
            fused_features.append(fused_features2)
            mu0.append(mu_01)
            mu1.append(mu_11)
            logvar0.append(logvar_01)
            logvar1.append(logvar_11)
...
class CustomLoss(nn.Module):
    ...
    def forward(self, gcn_output, model_output, target_labels, fused_features, mu_0,
        ...
        mse_loss1 = F.mse_loss(fused_features[0], gcn_output[0].view(
            gcn_output[0].size(0), -1), reduction='sum')
        # 3.2 KL散度损失
        kld_01 = -0.5 * torch.sum(1 + logvar_0[0] - mu_0[0].pow(2) - logvar_0[0].exp())
        kld_11 = -0.5 * torch.sum(1 + logvar_1[0] - mu_1[0].pow(2) - logvar_1[0].exp())
        if len(gcn_output) == 2:
            mse_loss2 = F.mse_loss(fused_features[1], gcn_output[1].view(
                gcn_output[1].size(0), -1), reduction='sum')
            kld_02 = -0.5 * torch.sum(1 + logvar_0[1] - mu_0[1].pow(2) - logvar_0[1].exp())
            kld_12 = -0.5 * torch.sum(1 + logvar_1[1] - mu_1[1].pow(2) - logvar_1[1].exp())
        else:
            mse_loss2 = 0
            kld_02 = 0
            kld_12 = 0
        ...
        # 3.3 总VAE损失
        loss_vae = mse_loss1 + mse_loss2 + kld_01 + kld_11 + kld_02 + kld_12
...
def train(model, data, optimizer):
    ...
    similarity1 = calculate_similarity(fused_features_train[0], gcn_matrix_train[0])
    if len(gcn_matrix_train) == 2:
        similarity2 = calculate_similarity(fused_features_train[1], gcn_matrix_train[1])
...
def test(model, data):
    ...
    similarity1 = calculate_similarity(fused_features_test[0], gcn_matrix_test[0])
    if len(gcn_matrix_test) == 2:
        similarity2 = calculate_similarity(fused_features_test[1], gcn_matrix_test[1])
        similarity = (similarity1 + similarity2) / 2
    else:
        similarity = similarity1
    # 计算准确率
...
def main_experiment():
    ...
    for dataset_name in datasets:
        ...
        for model_type in teacher_models:
            ...
            log_dir = f'/home/ff/code/spike/tensorboard_logs/{dataset_name}/{model_type}_2Layers_conduct_every'
            create_log_dir(log_dir)  # Ensure the log directory exists
            writer = SummaryWriter(log_dir=log_dir)

            # Create output directory for this model and dataset
            output_dir = f'/home/ff/code/spike/node_classification/results_{dataset_name}_{model_type}_2Layers_conduct_every'
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
